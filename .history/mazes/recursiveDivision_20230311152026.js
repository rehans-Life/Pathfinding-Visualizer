...
export function randomMaze(grid, setGrid) {
  ...
  animateWalls(firstRow,boundryColumns, lastRow, newGrid, setGrid);
...
function animateWalls(firstRow,boundryColumns,lastRow, grid, setGrid) {
  for (let i = 0; i < firstRow?.length; i++) {
    if (i === firstRow.length - 1) {
      setTimeout(() => {
        animateBoundry(boundryColumns, grid, setGrid);
      }, 30 * i);
    }
    setTimeout(() => {
      let newGrid = grid.slice();
