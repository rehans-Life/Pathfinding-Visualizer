...
export function randomMaze(grid, setGrid) {
  ...
  for (let i = 0; i < COLS; i++) {
    ...
    boundryNodes.push(firstRowNode);
  }

  for (let i = 0; i < ROWS; i++) {
    let firstColNode = newGrid[i][0];
    let lastColNode = newGrid[i][COLS - 1];
    boundryNodes.push(firstColNode);
    boundryNodes.push(lastColNode);
  }

  for (let i = 0; i < COLS; i++) {
    let lastRowNode = newGrid[ROWS - 1][i];
    boundryNodes.push(lastRowNode);
  }

  animateWalls(boundryNodes, newGrid, setGrid);
...
function animateWalls(boundryNodes, grid, setGrid) {
  for (let i = 0; i < boundryNodes?.length; i++) {
    setTimeout(() => {
      let newGrid = grid.slice();

      let node = boundryNodes[i];
      let newNode = {
        ...node,
        isWall: true,
      };
