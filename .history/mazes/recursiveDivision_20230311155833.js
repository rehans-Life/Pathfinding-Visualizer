...
export function randomMaze(grid, setGrid) {
  ...
  animateWalls(visitedNodes, newGrid, setGrid);
...
function divide(grid, minRow, maxRow, minCol, maxCol, direction, visitedNodes) {
  ...
  if (direction === "row") {
    ...
    if (maxRow - (randomRow + 2) < maxCol - minCol) {
      ...
      divide(grid, randomRow + 2, maxRow, minCol, maxCol, "row", visitedNodes);
    }
  } else {
    let possibleCols = [];
    ...
    if (maxRow - minRow < randomCol - 2 - minCol) {
      ...
      divide(grid, minRow, maxRow, minCol, randomCol - 2, "row", visitedNodes);
    }
...
function animateWalls(visitedNodes, grid, setGrid) {
  for (let i = 0; i < visitedNodes?.length; i++) {
    setTimeout(() => {
      ...
      let newNode = {
        ...node,
        isWall: node.row !== END_ROW || node.col !== END_COL ? true : false,
      };
