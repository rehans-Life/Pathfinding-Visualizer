...
export function randomMaze(grid, setGrid) {
  ...
  while (wallList.length) {
    ...
    if (visitedAdjNodes.length) {
      let randomAdjNodeIndex = Math.floor(Math.random * visitedAdjNodes.length);
      let randomAdjNode = visitedAdjNodes[randomAdjNodeIndex];
      randomAdjNode.isWall = false;
      for (let i = 0; i < 4; i++) {
        let adjRow = startNode.row + delRow[i];
        let adjCol = startNode.col + delCol[i];
        ...
        if (isValid(adjRow, adjCol)) {
          let adjNode = newGrid[adjRow][adjCol];
          if (adjNode.distance !== 1) wallList.push(adjNode);
        }
      }
    }
  }
