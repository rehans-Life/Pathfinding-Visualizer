...
function bidirectional(grid, src, target) {
  ...
  while (forwardQueue.length && backwardQueue.length) {
    let forwardintersectingNode = bfs(
      grid,
      forwardQueue,
      forwardVisitedNodes,
      backwardVisitedNodes
    );
    if (forwardintersectingNode) {
      return [
        answer(forwardVisitedNodes, backwardVisitedNodes),
        genertatePath(
          forwardVisitedNodes,
          backwardVisitedNodes,
          forwardintersectingNode
        ),
      ];
    }
    let backwardIntersectingNode = bfs(
      grid,
      backwardQueue,
      backwardVisitedNodes,
      forwardVisitedNodes
    );
    if (backwardIntersectingNode) {
      return [
        answer(forwardVisitedNodes, backwardVisitedNodes),
        genertatePath(
          forwardVisitedNodes,
          backwardVisitedNodes,
          backwardIntersectingNode
        ),
      ];
    }
  }
...
function genertatePath(
  ...
  let forwardPath = [];
  let forwardNode = intersectingingNode;
  while (forwardNode) {
