...
function bidirectional(grid, src, target) {
  ...
  while (forwardQueue.length && backwardQueue.length) {
    ...
    if (addNeighbours(forwardNode, grid, forwardQueue, visitedNodes)) {
      return visitedNodes;
    }

    let backwardQueue = backwardQueue.shift();
    if (addNeighbours(backwardQueue, grid, backwardQueue, visitedNodes)) {
      return visitedNodes;
    }
  }
  return visitedNodes;
...
function addNeighbours(node, grid, queue, visitedNodes) {
  let delRow = [-1, +1, 0, 0];
  let delCol = [0, 0, -1, +1];
