...
function bidirectional(grid, src, target) {
  ...
  while (forwardQueue.length && backwardQueue.length) {
    let forwardintersectingNodes = bfs(
      grid,
      forwardQueue,
      forwardVisitedNodes,
      backwardVisitedNodes
    );
    if (forwardintersectingNodes) {
      return [
        answer(forwardVisitedNodes, backwardVisitedNodes),
        genertatePath(
          forwardVisitedNodes,
          backwardVisitedNodes,
          forwardintersectingNodes
        ),
      ];
    }
...
function genertatePath(
  forwardVisitedNodes,
  backwardVisitedNodes,
  intersectingingNodes
) {
  let forwardPath = [];
  let forwardNode = intersectingingNode[0];
  while (forwardNode) {
