...
function bidirectional(grid, src, target) {
  ...
  while (forwardQueue.length && backwardQueue.length) {
    ...
    addNeighbours(forwardNode, grid, forwardQueue, visitedNodes, "Forward");
    let backwardNode = backwardQueue.shift();
    if (
      addNeighbours(backwardNode, grid, backwardQueue, visitedNodes, "Backward")
    ) {
      return visitedNodes;
    }
  }
...
function addNeighbours(node, grid, queue, visitedNodes, direction) {
  let delRow = [-1, +1, 0, 0];
  let delCol = [0, 0, -1, +1];
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (isValid(adjRow, adjCol)) {
      ...
      if (direction === "Backward" && adjNode.isIntersecting) {
        return true;
      }
